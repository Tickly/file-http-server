...
function HttpServer(options) {
  ...
  if (!proxyAll) {
    before.push(require('./delete-handler')({
      root: this.root,
  ...
  // CLI / README 约定 timeout 单位为秒；Node.js server.setTimeout 需要毫秒
  if (isNaN(options.timeout) || isNaN(parseFloat(options.timeout))) {
    this.server.setTimeout(120 * 1000);
  } else {
    this.server.setTimeout(Math.max(0, Number(options.timeout) * 1000));
  }
